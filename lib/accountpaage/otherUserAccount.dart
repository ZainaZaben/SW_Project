...
class _otherUserAccountPageState extends State<otherUserAccountPage> {
  ...
  Future<void> initializeData() async {
    try {
      ...
      if (registerID != null) {
        ...
        print('reservedArray: $reviews');
        if (widget.flagpayment) {
          show(registerid);
        }
      } else {
  ...
  Future<void> UpdatereviewDone({
    required String sellerId,
    required String purchaserId,
    required String itemId,
    required String reviewDone,
  }) async {
    final ipAddress = await getLocalIPv4Address();
    final url = Uri.parse('http://$ipAddress:3000/updateReviewDone');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        // Data sent successfully
        print('reviewDone updated successfully');
      } else if (response.statusCode == 400) {
        print("400 error");
      } else {
        print('HTTP error: ${response.statusCode}');
  ...
  ////************* *///
  Future<List<Map<String, dynamic>>> showReviewOther(
      String original_userID) async {
    final ipAddress = await getLocalIPv4Address();
    final String apiUrl =
        "http://$ipAddress:3000/showReviewOtherUser/$original_userID";
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                GestureDetector(
                  onTap: () async {
                    reviews = await showReviewOther(widget.userRegisterID);
                    showReview(review_userID);
                  },
                  child: Container(
                    width: 50,
                    height: 50,
                        ...
                        Container(
                          ...
                          child: ElevatedButton(
                            onPressed: () async {
                              ...
                              await UpdatereviewDone(
                                  sellerId: widget.userRegisterID,
                                  purchaserId: review_userID,
                                  itemId: widget.itemId,
                                  reviewDone: "1");
                            },
                            style: ElevatedButton.styleFrom(
